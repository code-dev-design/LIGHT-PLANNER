...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  try:
   ...
   if path=='/api/health':return self.send_json({'status':'ok','engine':f"PyMuPDF {getattr(fitz,'__version__','')}",'mode':'poc01-normalized-vector-pdf-import','schema':3})
   ...
   if m:
    pid,ps=m.groups();pr=PROJECTS.get(pid)
    if not pr:return self.send_json({'detail':'Project not found'},404)
    pi=int(ps);cache=CACHE_DIR/f'{pid}-vectors-v3-{pi}.json.gz'
    if cache.exists():
